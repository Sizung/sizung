class AddArchivalForConversationObjects < ActiveRecord::Migration
  def up
    drop_view :conversation_objects

    [:agenda_items, :deliverables, :comments].each do |table|
      add_column table, :archive_number, :string
  ...
  def down
    ...
    [:agenda_items, :deliverables, :comments].each do |table|
      remove_column table, :archive_number
