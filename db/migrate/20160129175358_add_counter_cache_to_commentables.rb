class AddCounterCacheToCommentables < ActiveRecord::Migration
  def up
    drop_view :conversation_objects

    [:conversations, :agenda_items, :deliverables].each do |t|
      add_column t, :comments_count, :integer, null: false, default: 0
    end
    ...
    [Conversation, AgendaItem, Deliverable].each do |klass|
      klass.all.pluck(:id).each do |id|
        klass.reset_counters(id, :comments)
      end
    end
  end
  ...
  def down
    ...
    [:conversations, :agenda_items, :deliverables].each do |t|
      remove_column t, :comments_count
    end
