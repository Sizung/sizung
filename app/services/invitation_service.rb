class InvitationService
  def invite(invited_email_address, current_organization, current_inviter, &block)
    ...
    if existing_user && existing_user.accepted_or_not_invited?
      ...
      existing_user.organization_members.create(organization: current_organization)
      ...
      User.invite!({email: invited_email_address}, current_inviter) do |u|
        u.organization_members.build(organization: current_organization)
      end
    end
  end
end
